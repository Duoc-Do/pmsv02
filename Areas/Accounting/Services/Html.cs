...
using WebApp.Areas.Accounting.Services.CodeFormatter;
...
namespace WebApp.Areas.Accounting.Services
{
    ...
    public static class HtmlHelpers
    {
        ...
        public static MvcHtmlString svEditorFor<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression, object htmlAttributes)
        {
            ...
            {
                case "datetime":
                    var builderdiv2 = new TagBuilder("div");
                    builderdiv2.AddCssClass("input-group date");
                    builderdiv2.InnerHtml = builder.ToString();
                    builderdiv2.InnerHtml += @"<span class='input-group-addon'><span class='glyphicon glyphicon-calendar'></span></span>";

                    //builderdiv.InnerHtml = strlabel.ToString();
                    builderdiv.InnerHtml += builderdiv2.ToString();
                    builderdiv.InnerHtml += errormessage;
        ...
        public static MvcHtmlString bsLabelFor<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression, object htmlAttributes,string labelvalue)
        {
            ...
            builder.AddCssClass("control-label");
            builder.InnerHtml = labelvalue??DisplayName;
            return MvcHtmlString.Create(builder.ToString());
        }

        public static MvcHtmlString bsSpanFor<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression)
        {
            return bsSpanFor(html, expression, null);
        }


        public static MvcHtmlString bsSpanFor<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression, object htmlAttributes)
        {
        ...
        public static MvcHtmlString bsEditorFor<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression, object htmlAttributes)
        {
            ...
            var arrMeta = Services.GlobalMeta.GetMetaObject(metadata.ContainerType.Name).GetMetaByColumnName(metadata.PropertyName);
            string datatype = Services.ExConvert.Sqltype2Systemtype(arrMeta.DATA_TYPE);
            if (arrMeta.DATA_TYPE==null && arrMeta.UType && !string.IsNullOrEmpty(arrMeta.FormatType))
            {
                datatype = arrMeta.FormatType.ToLower();
            }
            ...
            {
                builder.MergeAttribute("style", "text-align:right;");
                builder.MergeAttribute("decimaldigits", "0");
            }
        ...
        public static MvcHtmlString bsInputFor<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression)
        {
            return bsInputFor(html, expression, null);
        }

        public static MvcHtmlString bsInputFor<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression, object htmlAttributes)
        {
        ...
        public static MvcHtmlString bsDropDownList2For<TModel, TValue>(this HtmlHelper<TModel> html, Expression<Func<TModel, TValue>> expression, IEnumerable<SelectListItem> selectlist, string optionLabel,string labelvalue)
        {
            TagBuilder div = new TagBuilder("div");
            div.AddCssClass("form-group");

            MvcHtmlString label = bsLabelFor(html, expression,null,labelvalue);
        ...
        public static MvcHtmlString svDataRow<TModel>(this HtmlHelper htmlHelper, TModel row, Dictionary<string, Models.SysTableDetailView> columns)
        {
            ...
            {
                object value=null;
                if (!column.Value.UType)
                {
                    value = row.GetType().GetProperty(column.Value.ColumnName).GetValue(row, null) ?? String.Empty;
                }
        ...
        //lưới liệt kê danh sách có phân trang dành cho nhiều khóa
        public static MvcHtmlString svGridList2(this HtmlHelper html, IEnumerable<dynamic> source, string keyfield, string tablename)
        {
        ...
        public static MvcHtmlString svGridVoucher(this HtmlHelper html, IEnumerable<dynamic> source, string keyfield, string defaultkeyvalue, object newitem)
        {
            ...
            foreach (var item in columns)
            {
                ...
                string datatype = Services.ExConvert.Sqltype2Systemtype(item.Value.DATA_TYPE); //row.GetType().GetProperty(column.Value.ColumnName).PropertyType.ToString();

                if (datatype=="datetime")
                {
                    th.MergeAttribute("style", string.Format("width: {0}px;", item.Value.GridViewWidth + 34),true);
                }
        ...
        $(tbbody).find('tr[rownumber=-1]').find('input').val('');";
            ...
            fc += "$(tbbody).find('tr[rownumber=-1] select[isautocomplete = \"isautocomplete\"]').each(function (i) {var selectize = $(this)[0].selectize;if(typeof selectize != 'undefined') {selectize.clear();selectize.clearOptions();};});";
            fc += @"$(newrow).attr({rownumber: rowcount });
        $(tbbody).find('tr:last').after(newrow)
        $(tbbody).find('tr:last').show();
        $(tbbody).find('tr:last>td').each(function (index, value) {

            //huy selectize
            //$(value).find('.selectize-control:visible').first().remove();
            var  selectize =  $(value).find('.selectize-control:visible').first();
            $(selectize).parent().find('select').show();
            $(selectize).remove();

            var col = $(value).find('input,select').first();
            ...
            fc += "$(tbbody).find('tr:last select[isautocomplete = \"isautocomplete\"]:visible').each(function (i) {";

            UrlHelper u = new UrlHelper(html.ViewContext.RequestContext);
            string url = u.Action("autocomplete", "Services", new { area = "Accounting" });
            fc += string.Format("$(this).lookup2('{0}'){1}", url, ";}); ");
        ...
        private static MvcHtmlString AddRowNew(this HtmlHelper html, object row, Dictionary<string, Models.SysTableDetailView> columns, string tablename, string keyfield, int rownumber)
        {
            ...
            foreach (var column in columns)
            {
                ...
                {
                    col = new TagBuilder("select");
                    col.MergeAttribute("isautocomplete", "isautocomplete", true);

                    var optiontag = new TagBuilder("option");
                    optiontag.MergeAttribute("value", value.ToString(), true);
                    col.InnerHtml = optiontag.ToString();
                }
                else
                {
                    col.MergeAttribute("type", "text");
                ...
                if (datatype == "datetime")
                {
                    var builderdiv2 = new TagBuilder("div");
                    builderdiv2.AddCssClass("input-group date");
                    //builderdiv2.MergeAttribute("data-date-show-on-focus", "false");
                    //builderdiv2.MergeAttribute("data-provide", "datepicker");
                    builderdiv2.InnerHtml = col.ToString();
                    builderdiv2.InnerHtml += @"<span class='input-group-addon no-border' style='margin:0;padding:0;'><span class='glyphicon glyphicon-calendar'></span></span>";
                    td.InnerHtml = builderdiv2.ToString();
                }
                else
                {
                    td.InnerHtml = col.ToString();
            ...
            //div.InnerHtml += @"<button type='button' class='sv-button-addline' onclick='addnew(this)'>Thêm dòng</button>";
            var tdedit = new TagBuilder("td") { InnerHtml = string.Format(@"<button type='button' class='btn btn-primary' onclick='senvietgridaddrow(this)'><span class='fa fa-plus-circle'></span></button><a href='javascript:;' class='senviet-grid-btn-addrow'></a>") };
        ...
        private static MvcHtmlString AddRow(this HtmlHelper html, object row, Dictionary<string, Models.SysTableDetailView> columns, string tablename, string keyfield, int rownumber)
        {
            ...
            foreach (var column in columns)
            {
                ...
                if (datatype == "int")
                {
                    ...
                    {
                        col.MergeAttribute("value", "0");
                    }
                    else
                    {
                        col.MergeAttribute("value", Convert.ToInt32(value.ToString()).ToString());
                    }
