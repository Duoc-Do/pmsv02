...
{
    public partial class AppForwardEntryTable : SenVietListObject
    {
        public AppForwardEntryTable(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "AppForwardEntryTable";
            this._metaname = "AppForwardEntryView";
            this._keyfield = "ForwardEntryID";
            this._storeNameI = @"sp_AppForwardEntryTableI @ForwardEntryID OUTPUT,@ForwardEntryCode,@Name,@AccountDebitLineID,@AccountCreditLineID,@DebitToCredit,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime";
            this._storeNameU = @"sp_AppForwardEntryTableU @ForwardEntryID OUTPUT,@ForwardEntryCode,@Name,@AccountDebitLineID,@AccountCreditLineID,@DebitToCredit,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime,@Original_ForwardEntryID";
            this._storeNameD = @"sp_AppForwardEntryTableD {0}";
            base.InitObject();
        }

        public List<Models.AppForwardEntryView> GetData()
        {
            var model = Services.GridHelper.GetResults(this._request, this._metaname, this._db.AppForwardEntryViews);
            return model;
        }

        public Models.AppForwardEntryView GetById(int Id)
        {
            return this._db.AppForwardEntryViews.SingleOrDefault(m => m.ForwardEntryID == Id);
        }

        public Models.AppForwardEntryView GetNew(int? id)
        {
            if (id != null) return GetById(id ?? 0);
            return new Models.AppForwardEntryView();
        }

        public Models.AppForwardEntryView GetEdit(int id)
        {
            return GetById(id);
        }

        public Models.AppForwardEntryView GetDelete(int id)
        {
            return GetById(id);
        }

        public int Insert(Models.AppForwardEntryView data)
        {
            try
            {
                this.Validate(data);
        ...
        public int Update(Models.AppForwardEntryView data)
        {
            try
            {
                this.Validate(data);
