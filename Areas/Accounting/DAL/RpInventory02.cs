...
{
    public partial class RpInventory02 : SenVietReportObject
    {
        public RpInventory02(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "RpInventory02";
            this._metaname = "RpInventory02";
            this._keyfield = "StockCardID";
            this._storeName = @"sp_RpInventory02 '{0}','{1}','{2}','{3}'";
        ...
        public List<Models.RpInventory02> GetData()
        {
            ...
            List<Models.RpInventory02> result = null;

            if (!this.IsSession())
            {
                result = this._db.Database.SqlQuery<Models.RpInventory02>(strProcedure).ToList();
            }
            else
            {
                var resultsession = this.GetDataCache<Models.RpInventory02>();
            ...
            var model = Services.GridHelper.GetResults(this._request, this._metaname, result.AsQueryable<Models.RpInventory02>());
        ...
        public override byte[] ExportToExcel()
        {
            ...
            {
                Export.ExportToXlsx(stream, this.GetDataCacheAll<Models.RpInventory02>(), this._metaobject.GetMetaTable());
