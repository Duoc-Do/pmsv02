...
{
    public partial class AppCustomerBalanceTable : SenVietListObject
    {
        public AppCustomerBalanceTable(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "AppCustomerBalanceTable";
            this._metaname = "AppCustomerBalanceView";
            this._keyfield = "CustomerBalanceID";
            this._storeNameI = @"sp_AppCustomerBalanceTableI @CustomerBalanceID OUTPUT,@BalanceDate,@CustomerID,@AccountID,@Debit,@Credit,@DebitFC,@CreditFC,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime";
            this._storeNameU = @"sp_AppCustomerBalanceTableU @CustomerBalanceID OUTPUT,@BalanceDate,@CustomerID,@AccountID,@Debit,@Credit,@DebitFC,@CreditFC,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime,@Original_CustomerBalanceID";
            this._storeNameD = @"sp_AppCustomerBalanceTableD {0}";
            base.InitObject();
        }

        public List<Models.AppCustomerBalanceView> GetData()
        {
            var model = Services.GridHelper.GetResults(this._request, this._metaname, this._db.AppCustomerBalanceViews);
            return model;
        }

        public Models.AppCustomerBalanceView GetById(int Id)
        {
            return this._db.AppCustomerBalanceViews.SingleOrDefault(m => m.CustomerBalanceID == Id);
        }

        public Models.AppCustomerBalanceView GetNew(int? id)
        {
            if (id != null) return GetById(id ?? 0);
            return new Models.AppCustomerBalanceView();
        }

        public Models.AppCustomerBalanceView GetEdit(int id)
        {
            return GetById(id);
        }

        public Models.AppCustomerBalanceView GetDelete(int id)
        {
            return GetById(id);
        }
        public int Insert(Models.AppCustomerBalanceView data)
        {
            try
            {
                this.Validate(data);
        ...
        public int Update(Models.AppCustomerBalanceView data)
        {
            try
            {
                this.Validate(data);
