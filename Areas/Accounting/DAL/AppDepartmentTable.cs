...
{
    public partial class AppDepartmentTable : SenVietListObject
    {
        public AppDepartmentTable(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "AppDepartmentTable";
            this._metaname = "AppDepartmentTable";
            this._keyfield = "DepartmentID";
            this._storeNameI = @"sp_AppDepartmentTableI @DepartmentID OUTPUT,@ParentID,@DepartmentCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime";
            this._storeNameU = @"sp_AppDepartmentTableU @DepartmentID OUTPUT,@ParentID,@DepartmentCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime,@Original_DepartmentID";
            this._storeNameD = @"sp_AppDepartmentTableD {0}";
            base.InitObject();
        }

        public List<Models.AppDepartmentTable> GetData()
        {
            var model = Services.GridHelper.GetResults(this._request, this._metaname, this._db.AppDepartmentTables);
            return model;
        }

        public Models.AppDepartmentTable GetById(int Id)
        {
            return this._db.AppDepartmentTables.SingleOrDefault(m => m.DepartmentID == Id);
        }

        public Models.AppDepartmentTable GetNew(int? id)
        {
            if (id != null) return GetById(id ?? 0);
            return new Models.AppDepartmentTable();
        }

        public Models.AppDepartmentTable GetEdit(int id)
        {
            return GetById(id);
        }

        public Models.AppDepartmentTable GetDelete(int id)
        {
            return GetById(id);
        }
        public int Insert(Models.AppDepartmentTable data)
        {
            try
            {
                this.Validate(data);
        ...
        public int Update(Models.AppDepartmentTable data)
        {
            try
            {
                this.Validate(data);
