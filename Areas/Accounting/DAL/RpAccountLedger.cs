...
{
    public partial class RpAccountLedger : SenVietReportObject
    {
        public RpAccountLedger(HttpRequestBase request) : base(request) { }
        public override void InitObject()
        {
            this._businesscode = "RpAccountLedger";
            this._metaname = "RpAccountLedger";
            this._keyfield = "DocumentID";
            this._storeName = @"sp_RpAccountLedger '{0}','{1}','{2}'";
            base.InitObject();
        }
        ...
        public List<Models.RpAccountLedger> GetData()
        {
            ...
            List<Models.RpAccountLedger> result = null;
            if (!this.IsSession())
            {
                result = this._db.Database.SqlQuery<Models.RpAccountLedger>(strProcedure).ToList();
            }
            else
            {
                var resultsession = this.GetDataCache<Models.RpAccountLedger>();
            ...
            var model = Services.GridHelper.GetResults(this._request, this._metaname, result.AsQueryable<Models.RpAccountLedger>());
        ...
        public override byte[] ExportToExcel()
        {
            ...
            {
                Export.ExportToXlsx(stream, this.GetDataCacheAll<Models.RpAccountLedger>(), this._metaobject.GetMetaTable());
