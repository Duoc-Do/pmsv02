...
{
    public partial class AppUnitOfMeasureTable : SenVietListObject
    {
        public AppUnitOfMeasureTable(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "AppUnitOfMeasureTable";
            this._metaname = "AppUnitOfMeasureView";
            this._keyfield = "UOMID";
            this._storeNameI = @"sp_AppUnitOfMeasureTableI @UOMID OUTPUT,@UOMCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime";
            this._storeNameU = @"sp_AppUnitOfMeasureTableU @UOMID OUTPUT,@UOMCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime,@Original_UOMID";
            this._storeNameD = @"sp_AppUnitOfMeasureTableD {0}";
            base.InitObject();
        }

        public List<Models.AppUnitOfMeasureView> GetData()
        {
            var model = Services.GridHelper.GetResults(this._request, this._metaname, this._db.AppUnitOfMeasureViews);
            return model;
        }

        public Models.AppUnitOfMeasureView GetById(int Id)
        {
            return this._db.AppUnitOfMeasureViews.SingleOrDefault(m => m.UOMID == Id);
        }

        public Models.AppUnitOfMeasureView GetNew(int? id)
        {
            if (id != null) return GetById(id ?? 0);
            return new Models.AppUnitOfMeasureView() { IsActive=true};
        }

        public Models.AppUnitOfMeasureView GetEdit(int id)
        {
            return GetById(id);
        }

        public Models.AppUnitOfMeasureView GetDelete(int id)
        {
            return GetById(id);
        }
        public int Insert(Models.AppUnitOfMeasureView data)
        {
            try
            {
                this.Validate(data);
        ...
        public int Update(Models.AppUnitOfMeasureView data)
        {
            try
            {
                this.Validate(data);
