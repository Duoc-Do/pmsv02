...
{
    public partial class RpStockCard : SenVietReportObject
    {
        public RpStockCard(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "RpStockCard";
            this._metaname = "RpStockCard";
            this._keyfield = "StockCardID";
            this._storeName = @"sp_RpStockCard '{0}','{1}','{2}','{3}'";
        ...
        public List<Models.RpStockCard> GetData()
        {
            ...
            List<Models.RpStockCard> result = null;

            if (!this.IsSession())
            {
                result = this._db.Database.SqlQuery<Models.RpStockCard>(strProcedure).ToList();
            }
            else
            {
                var resultsession = this.GetDataCache<Models.RpStockCard>();
            ...
            var model = Services.GridHelper.GetResults(this._request, this._metaname, result.AsQueryable<Models.RpStockCard>());
        ...
        public override byte[] ExportToExcel()
        {
            ...
            {
                Export.ExportToXlsx(stream, this.GetDataCacheAll<Models.RpStockCard>(), this._metaobject.GetMetaTable());
