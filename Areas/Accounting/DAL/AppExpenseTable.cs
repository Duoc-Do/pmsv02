...
{
    public partial class AppExpenseTable : SenVietListObject
    {
        public AppExpenseTable(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "AppExpenseTable";
            this._metaname = "AppExpenseTable";
            this._keyfield = "ExpenseID";
            this._storeNameI = @"sp_AppExpenseTableI @ExpenseID OUTPUT,@ExpenseCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime";
            this._storeNameU = @"sp_AppExpenseTableU @ExpenseID OUTPUT,@ExpenseCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime,@Original_ExpenseID";
            this._storeNameD = @"sp_AppExpenseTableD {0}";
            base.InitObject();
        }

        public List<Models.AppExpenseTable> GetData()
        {
            var model = Services.GridHelper.GetResults(this._request, this._metaname, this._db.AppExpenseTables);
            return model;
        }

        public Models.AppExpenseTable GetById(int Id)
        {
            return this._db.AppExpenseTables.SingleOrDefault(m => m.ExpenseID == Id);
        }

        public Models.AppExpenseTable GetNew(int? id)
        {
            if (id != null) return GetById(id ?? 0);
            return new Models.AppExpenseTable() { IsActive=true};
        }

        public Models.AppExpenseTable GetEdit(int id)
        {
            return GetById(id);
        }

        public Models.AppExpenseTable GetDelete(int id)
        {
            return GetById(id);
        }
