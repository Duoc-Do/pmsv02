...
{
    public partial class AppFixedAssetGroupTable : SenVietListObject
    {
        public AppFixedAssetGroupTable(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "AppFixedAssetGroupTable";
            this._metaname = "AppFixedAssetGroupTable";
            this._keyfield = "FixedAssetGroupID";
            this._storeNameI = @"sp_AppFixedAssetGroupTableI @FixedAssetGroupID OUTPUT,@FixedAssetGroupCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime";
            this._storeNameU = @"sp_AppFixedAssetGroupTableU @FixedAssetGroupID OUTPUT,@FixedAssetGroupCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime,@Original_FixedAssetGroupID";
            this._storeNameD = @"sp_AppFixedAssetGroupTableD {0}";
            base.InitObject();
        }

        public List<Models.AppFixedAssetGroupTable> GetData()
        {
            var model = Services.GridHelper.GetResults(this._request, this._metaname, this._db.AppFixedAssetGroupTables);
            return model;
        }

        public Models.AppFixedAssetGroupTable GetById(int Id)
        {
            return this._db.AppFixedAssetGroupTables.SingleOrDefault(m => m.FixedAssetGroupID == Id);
        }

        public Models.AppFixedAssetGroupTable GetNew(int? id)
        {
            if (id != null) return GetById(id ?? 0);
            return new Models.AppFixedAssetGroupTable() { IsActive=true};
        }

        public Models.AppFixedAssetGroupTable GetEdit(int id)
        {
            return GetById(id);
        }

        public Models.AppFixedAssetGroupTable GetDelete(int id)
        {
            return GetById(id);
        }
        public int Insert(Models.AppFixedAssetGroupTable data)
        {
            try
            {
                this.Validate(data);
        ...
        public int Update(Models.AppFixedAssetGroupTable data)
        {
            try
            {
                this.Validate(data);
