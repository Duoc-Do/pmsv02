...
{
    public partial class AppExchangeRateTable : SenVietListObject
    {
        public AppExchangeRateTable(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "AppExchangeRateTable";
            this._metaname = "AppExchangeRateView";
            this._keyfield = "ExchangeRateID";
            this._storeNameI = @"sp_AppExchangeRateTableI @ExchangeRateID OUTPUT,@CurrencyID,@FromDate,@Value,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime";
            this._storeNameU = @"sp_AppExchangeRateTableU @ExchangeRateID OUTPUT,@CurrencyID,@FromDate,@Value,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime,@Original_ExchangeRateID";
            this._storeNameD = @"sp_AppExchangeRateTableD {0}";
            base.InitObject();
        }

        public List<Models.AppExchangeRateView> GetData()
        {
            var model = Services.GridHelper.GetResults(this._request, this._metaname, this._db.AppExchangeRateViews);
            return model;
        }

        public Models.AppExchangeRateView GetById(int Id)
        {
            return this._db.AppExchangeRateViews.SingleOrDefault(m => m.ExchangeRateID == Id);
        }

        public Models.AppExchangeRateView GetNew(int? id)
        {
            if (id != null) return GetById(id ?? 0);
            return new Models.AppExchangeRateView();
        }

        public Models.AppExchangeRateView GetEdit(int id)
        {
            return GetById(id);
        }

        public Models.AppExchangeRateView GetDelete(int id)
        {
            return GetById(id);
        }

        public int Insert(Models.AppExchangeRateView data)
        {
            try
            {
                this.Validate(data);
        ...
        public int Update(Models.AppExchangeRateView data)
        {
            try
            {
                this.Validate(data);
