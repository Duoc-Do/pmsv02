...
{
    public partial class RpInventorySummaryMinMax : SenVietReportObject
    {
        public RpInventorySummaryMinMax(HttpRequestBase request) : base(request) { }
        public override void InitObject()
        {
            this._businesscode = "RpInventorySummaryMinMax";
            this._metaname = "RpInventorySummaryMinMax";
            this._keyfield = "DocumentID";
            this._storeName = @"sp_RpInventorySummaryMinMax '{0}','{1}'";
            base.InitObject();
        }
        ...
        public List<Models.RpInventorySummaryMinMax> GetData()
        {
            ...
            List<Models.RpInventorySummaryMinMax> result = null;
            if (!this.IsSession())
            {
                result = this._db.Database.SqlQuery<Models.RpInventorySummaryMinMax>(strProcedure).ToList();
            }
            else
            {
                var resultsession = this.GetDataCache<Models.RpInventorySummaryMinMax>();
            ...
            var model = Services.GridHelper.GetResults(this._request, this._metaname, result.AsQueryable<Models.RpInventorySummaryMinMax>());
        ...
        public override byte[] ExportToExcel()
        {
            ...
            {
                Export.ExportToXlsx(stream, this.GetDataCacheAll<Models.RpInventorySummaryMinMax>(), this._metaobject.GetMetaTable());
