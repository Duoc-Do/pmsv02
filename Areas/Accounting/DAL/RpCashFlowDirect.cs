...
{
    public partial class RpCashFlowDirect : SenVietReportObject
    {
        public RpCashFlowDirect(HttpRequestBase request) : base(request) { }
        public override void InitObject()
        {
            this._businesscode = "RpCashFlowDirect";
            this._metaname = "RpCashFlowDirect";
            this._keyfield = "DocumentID";
            this._storeName = @"sp_RpCashFlowDirect '{0}','{1}','{2}','{3}'";
            base.InitObject();
        }
        ...
        public List<Models.RpCashFlowDirect> GetData()
        {
            ...
            List<Models.RpCashFlowDirect> result = null;
            if (!this.IsSession())
            {
                result = this._db.Database.SqlQuery<Models.RpCashFlowDirect>(strProcedure).ToList();
            }
            else
            {
                var resultsession = this.GetDataCache<Models.RpCashFlowDirect>();
            ...
            var model = Services.GridHelper.GetResults(this._request, this._metaname, result.AsQueryable<Models.RpCashFlowDirect>());
        ...
        public override byte[] ExportToExcel()
        {
            ...
            {
                Export.ExportToXlsx(stream, this.GetDataCacheAll<Models.RpCashFlowDirect>(), this._metaobject.GetMetaTable());
