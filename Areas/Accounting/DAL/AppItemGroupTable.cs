...
{
    public partial class AppItemGroupTable : SenVietListObject
    {
        public AppItemGroupTable(HttpRequestBase request) : base(request) { }

        public override void InitObject()
        {
            this._businesscode = "AppItemGroupTable";
            this._metaname = "AppItemGroupView";
            this._keyfield = "ItemGroupID";
            this._storeNameI = @"sp_AppItemGroupTableI @ItemGroupID OUTPUT,@ItemGroupCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime";
            this._storeNameU = @"sp_AppItemGroupTableU @ItemGroupID OUTPUT,@ItemGroupCode,@Name,@IsActive,@CreatedBy,@CreatedDateTime,@ModifiedBy,@ModifiedDateTime,@Original_ItemGroupID";
            this._storeNameD = @"sp_AppItemGroupTableD {0}";
            base.InitObject();
        }

        public List<Models.AppItemGroupView> GetData()
        {
            var model = Services.GridHelper.GetResults(this._request, this._metaname, this._db.AppItemGroupViews);
            return model;
        }

        public Models.AppItemGroupView GetById(int Id)
        {
            return this._db.AppItemGroupViews.SingleOrDefault(m => m.ItemGroupID == Id);
        }

        public Models.AppItemGroupTable GetById2(int Id)
        {
            return this._db.AppItemGroupTables.SingleOrDefault(m => m.ItemGroupID == Id);
        }

        public Models.AppItemGroupView GetNew(int? id)
        {
            if (id != null) return GetById(id ?? 0);
            return new Models.AppItemGroupView() { IsActive=true};
        }

        public Models.AppItemGroupView GetEdit(int id)
        {
            return GetById(id);
        }

        public Models.AppItemGroupView GetDelete(int id)
        {
            return GetById(id);
        }
        public int Insert(Models.AppItemGroupView data)
        {
            try
            {
                this.Validate(data);
        ...
        public int Update(Models.AppItemGroupView data)
        {
            try
            {
                this.Validate(data);
