...
{
    public partial class RpOutputVAT : SenVietReportObject
    {
        public RpOutputVAT(HttpRequestBase request) : base(request) { }
        public override void InitObject()
        {
            this._businesscode = "RpOutputVAT";
            this._metaname = "RpOutputVAT";
            this._keyfield = "DocumentID";
            this._storeName = @"sp_RpOutputVAT '{0}','{1}'";
            base.InitObject();
        }
        ...
        public List<Models.RpOutputVAT> GetData()
        {
            ...
            List<Models.RpOutputVAT> result = null;
            if (!this.IsSession())
            {
                result = this._db.Database.SqlQuery<Models.RpOutputVAT>(strProcedure).ToList();
            }
            else
            {
                var resultsession = this.GetDataCache<Models.RpOutputVAT>();
            ...
            var model = Services.GridHelper.GetResults(this._request, this._metaname, result.AsQueryable<Models.RpOutputVAT>());
        ...
        public override byte[] ExportToExcel()
        {
            ...
            {
                Export.ExportToXlsx(stream, this.GetDataCacheAll<Models.RpOutputVAT>(), this._metaobject.GetMetaTable());
