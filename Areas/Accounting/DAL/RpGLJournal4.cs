...
{
    public partial class RpGLJournal4 : SenVietReportObject
    {
        public RpGLJournal4(HttpRequestBase request) : base(request) { }
        public override void InitObject()
        {
            this._businesscode = "RpGLJournal4";
            this._metaname = "RpGLJournal4";
            this._keyfield = "DocumentID";
            this._storeName = @"sp_RpGLJournal4 '{0}','{1}','{2}'";
            base.InitObject();
        }
        ...
        public List<Models.RpGLJournal4> GetData()
        {
            var datefrom = DateTime.Today;
            var dateto = DateTime.Today;
            Uti.Date.SetDateRange(this._request, out datefrom, out dateto);

            string DisplayNumberDebitList = this._request.Params["DisplayNumberDebitList"] != null ? this._request.Params["DisplayNumberDebitList"].ToString() : "111,112,131,113";
            string DisplayNumberCreditList = this._request.Params["DisplayNumberCreditList"] != null ? this._request.Params["DisplayNumberCreditList"].ToString() : "511,512";
            ...
            List<Models.RpGLJournal4> result = null;
            if (!this.IsSession())
            {
                result = this._db.Database.SqlQuery<Models.RpGLJournal4>(strProcedure).ToList();
            }
            else
            {
                var resultsession = this.GetDataCache<Models.RpGLJournal4>();
            ...
            var model = Services.GridHelper.GetResults(this._request, this._metaname, result.AsQueryable<Models.RpGLJournal4>());
        ...
        public override byte[] ExportToExcel()
        {
            ...
            {
                Export.ExportToXlsx(stream, this.GetDataCacheAll<Models.RpGLJournal4>(), this._metaobject.GetMetaTable());
