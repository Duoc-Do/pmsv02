...
namespace WebApp.Areas.Admin.Services
{
    public class GridHelper
    {
        ...
        public static List<T> GetResults<T>(HttpRequestBase request, string MetaName, ObjectQuery<T> query)
        {
            ...
            {
                //if (item.UType == true) continue;//không xử lí cột ảo do người dùng tạo
        ...
        public static List<T> GetResults<T>(HttpRequestBase request, string MetaName, IQueryable<T> query)
        {
            ...
            foreach (var item in MetaObject.MetaTable)
            {
                ...
                if (request.Params[fieldfilter] != null)
                {
                    ...
                    //Nếu kiểu GUID thì chèn điều kiện trực tiếp
                    if (datatype == "guid" && !string.IsNullOrEmpty(request.Params[fieldfilter]))
                    {
                        if (item.IS_NULLABLE == "YES")
                        {
                            query = query.Where(string.Format("it.{0}.Value.Equals(@0)", item.ColumnName), new object[] { Guid.Parse(request.Params[fieldfilter]) });
                        }
                        else
                        {
                            query = query.Where(string.Format("it.{0}.Equals(@0)", item.ColumnName), new object[] { Guid.Parse(request.Params[fieldfilter]) });
                        }
                    }
                }
                ...
                //trường hợp mở rộng lọc bằng tham số link
                if (request.Params[item.ColumnName] != null)
                {
                    string filter = "";
                    switch (datatype)
                    {
                        case "numeric":
                            filter = string.Format("it.{0}={1}", item.ColumnName, request.Params[item.ColumnName]);
                            query = query.Where(filter);
                            //.AsQueryable();
                            break;
                        case "guid":
                            //filter = string.Format("it.{0} = Guid\'{1}\'", item.ColumnName, request.Params[item.ColumnName]);
                            //query = query.Where(filter).AsQueryable();
                            if (item.IS_NULLABLE == "YES")
                            {
                                query = query.Where(string.Format("it.{0}.Value.Equals(@0)", item.ColumnName), new object[] { Guid.Parse(request.Params[item.ColumnName]) });
                            }
                            else
                            {
                                query = query.Where(string.Format("it.{0}.Equals(@0)", item.ColumnName), new object[] { Guid.Parse(request.Params[item.ColumnName]) });
                            }
                        ...
                        default:
                            if (string.IsNullOrEmpty(item.FilterExpression))
                            {
                                filter = string.Format("it.{0} like N'{1}%'", item.ColumnName, request.Params[item.ColumnName]);
                                query = query.Where(filter);
                                //.AsQueryable();
                            }
                            break;
        ...
        public static List<T> GetResultAlls<T>(HttpRequestBase request, string MetaName, IQueryable<T> query)
        {
            ...
            if (exspression != "()")
            {
                ...
                //query = query.Where("UserId.Equals(@0)", new object[] { Guid.Parse("52722ae2-ca9e-47fe-98dc-84a7e80d45ac") });
            }
        ...
        public class Filter
        {
            ...
            public FilterExpressionResult getExpression()
            {
                ...
                var CICC = (System.Globalization.CultureInfo)WebApp.Areas.Admin.Services.GlobalVariant.GetCultureInfo()["CICC"];
                            ...
                            for (int i = 0; i < 3; i++)
                            {
                                switch (i)
                                {
                                    case 0:
                                        ...
                                        {
                                            expressionString = string.Format("(it.{0} = {1})", Field, System.Convert.ToDecimal(arrexpression[i], CICC));
                                        }
                                        break;
                                    case 1:
                                        if (!string.IsNullOrWhiteSpace(arrexpression[i]))
                    ...
                    case "datetime":
                        ...
                        if (arrexpression != null)
                        {
                            for (int i = 0; i < 3; i++)
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(arrexpression[i]))
                                {
                                    DateTime d1 = DateTime.Parse(arrexpression[i], CICC);
